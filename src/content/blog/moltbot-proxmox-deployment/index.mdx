...
<Callout variant="tip">
**Why Self-Host?** Self-hosting Moltbot means your data stays on your infrastructure. No API calls to third-party services for sensitive operations, and you control exactly what the AI can access. For homelab enthusiasts, this is the way to go.
</Callout>
...
The Gateway service installs automatically as a LaunchAgent (macOS) or systemd service (Linux). Check status:

```bash
clawdbot status
```

For manual systemd setup on Linux:
...
Type=simple
User=your-username
ExecStart=/usr/local/bin/clawdbot gateway start
Restart=always
